...
export default function MatchHistoryCard({
  ...
  const team1 = match.details.info.participants.splice(0, 4);
  const team2 = match.details.info.participants.splice(5, 9);

  if (!currentUserObj && !team1 && !team2) {
  ...
  return (
    <div className="p-4">
      <Disclosure>
        ...
        <div className="px-2 ">
          <Disclosure.Panel className="text-gray-500 pt-1 bg-secondary">
            <div>
              <GameDetails match={team1}></GameDetails>
            </div>
            <div>
              <GameDetails match={team2}></GameDetails>
            </div>
          </Disclosure.Panel>
        </div>
      </Disclosure>
